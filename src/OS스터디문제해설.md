...
<br>**
**→ 지역성에는 공간의 지역성, 시간의 지역성 그리고 순차적 지역성이 있다.**
...
<br>**
**→ 페이징 기법에서 페이지의 처음 위치에서 해당 주소까지의 거리를 의미하는 D는 페이지의 크기와 프레임의 크기가 같다면 항상 같은 값을 갖지만, 둘의 크기가 다를 경우 D의 크기도 달라진다는 특징을 갖고 있다.** 
<br>
...
<br>
→ 변환 색인 버퍼를 사용할 때는 여러 개의 페이지 테이블 영역에서 일부만 가져오는 것이기 때문에, 이를 이용할 때는 모든 행을 검색하여 원하는 데이터를 찾아내야 한다는 특징을 갖고 있다. 
<br>**
...
<br>
→ 세그먼테이션에서 limit으로 프로세스의 크기 정보를 표시함으로써 메모리의 역할을 보호하는 역할을 수행하지만, 외부단편화는 세그먼테이션 자체가 갖는 문제이기 때문에 이를 해결해줄 수는 없다. 
<br>**
...
<br>
→ 캐시 연관 매핑은 캐시 메모리를 자유롭게 이용할 수 있다는 장점을 갖지만, 캐시 히트인지 미스인지 확인하기 위해 캐시의 모든 주소를 검색해야한다는 단점을 갖는다. 즉, 한 번의 검색만으로는 원하는 데이터를 찾아낼 수 없을 가능성이 더 높다.
<br>**
...
<br>
→ 읽기 작업을 한 적이 있는지 확인해주는 비트를 접근 비트라고 하며, 쓰기 작업을 한 적이 있는지 확인해주는 비트를 변경 비트라고 한다.
<br>
...
<br>
→ 메모리에 빈 프레임이 없다면 메모리에 있는 프레임 중 하나를 스왑 영역으로 내보낸 후에야 해당 페이지를 가져올 수 있다. 
<br>
...
<br>
→ 최적 페이지 알고리즘은 미래의 메모리 접근 패턴을 살푭고 대상 페이지를 선정하므로 성능은 가장 좋지만 사실상 구현이 불가능하다는 특징을 갖고 있다.** 
<br>
...
<br>
→ 2차 기회 페이지 교체 알고리즘은 큐를 사용하지만, 특정 페이지에 접근하여 페이지 부재 없이 성공할 경우 해당 페이지를 큐의 맨 뒤로 이동하여 대상 페이지에서 제외한다는 특징을 갖는다.** 
...
<br>
→ 물리 메모리의 크기를 늘리면 스레싱 발생 시점이 늦춰져 프로세스를 원만하기 실행할 수 있지만, 충분한 크기가 되면 그 이후에는 크기를 늘려도 작업 속도에 영향을 미치지 않는다.
<br>**
...
1) 물리 메모리의 프레임 크기는?
<br>
→ 물리 메모리의 프레임 크기는 페이지의 크기와 같은 2KB이다. 
2) 프레임 개수는?
<br>
→ 1GB / 2KB = 2^19개 
3) 프로세스 A의 주소 공간의 크기와 페이지의 개수는?
<br>
→ 32bit 주소체계에서 프로세스 A의 자체 주소 공간은 4GB이다. 4GB = 2^22B, 2KB = 2^11이므로 페이지의 개수는 주소 공간 / 페이지의 크기를 활용하면 2^22 / 2^11 = 2^11개의 페이지가 만들어진다.
4) 프로세스 A는 몇 개의 페이지로 구성되는가? (자연수로 표현, 반올림 사용 가능)
<br>
→ 54321B / 2KB = 27개의 페이지로 구성된다. 
5) 페이지 테이블의 항목 크기가 4B라면 프로세스 A의 페이지 테이블 크기는?
<br>
→ 페이지 테이블의 항목 개수는 페이지 개수와 동일하다. 따라서 4B X 2KB = 2^2B X 2^11B = 2^13B = 8KB이다.